...
1. 先读地图：**任何任务开始前（包括用户直接扔任务时）**先读 state.json；若是本会话首次响应，主动用一句话汇报上次进度和 next_context，再开始干活。**怀疑失忆时（上下文压缩）主动重读 state.json + 最近修改文件**，不确定时不猜。**涉及业务规则/表结构/API/枚举约定时，动手前先查对应文档（业务规则→docs/02-架构设计/业务规则说明.md；数据库表结构/API接口规范→README.md），不凭记忆。**
...
6. 修复问题立即登记（流程见全局 §2.1+）：本项目登记位置 f:\claude code\问题登记与复盘\问题登记簿.csv。
...
14. **保持一致性（双向）**：任何代码改动必须同步更新相关文档，任何文档改动必须反向检查代码是否仍匹配。
    - **代码改 → 文档查**：修改了 API 接口、数据模型、枚举值、配置文件后，必须检查 README.md / CLAUDE.md / 数据库设计.md / 详细设计开发文档.md 中对应的描述是否仍准确，不准确则同步改。
    - **文档改 → 代码查**：修改了文档中的字段定义、枚举值、业务规则、表结构后，必须检查代码（models / schemas / api / services）是否仍匹配，不匹配则同步改。
    - **文档改 → 文档查**：修改了某一处文档后，必须反向检查其他文档是否引用了同一内容，如有引用则同步更新，确保不出现"同一件事两处说法"的重复不一致。
    - 上述检查不要求提交报告，但必须在思维中完成。发现不一致时，**先修再动手**，不得带着不一致开始新任务。

## 结束会话时（用户说"结束会话/下班/收工"）
1. **必须更新 state.json**，记录4项：
...
| 字段                      | 枚举值                                                       |
| ------------------------- | ------------------------------------------------------------ |
| production_lines.code     | PL-CLASSROOM(大课堂), PL-DAXUEQING(大学情), PL-PLATFORM(平台产品线), PL-STEAM(科创教育), PL-EXAM-LANG(考试与语言学习产品线), PL-SPORT(智慧体育产品线), PL-MENTAL(智慧心育产品线), PL-SCIPOP(科普研究院), PL-READING(读写科技) — 括号内为 name 字段，非 code 的一部分 |
...
- todos：**project_id**（2026-07-28 首次建表）、parent_id(树形)、transferred_from(非transferred_from_id，外键→users.id)
...
- 数据可下钻：卡片→列表，环形图扇区→明细，列表/甘特/风险/待办行→详情弹窗。
